...
require("./core/password.php");
    ...
    if (isset($_FILES['profile_img'])) {
    ...
    } elseif (isset($_FILES['logo_img'])) {
        $file_name_array = explode(".", $_FILES['logo_img']['name']);
        $file_extension = array_pop($file_name_array);
        $file_name = $_FILES['logo_img']['name'] = uniqid() . "." . $file_extension;
        $temp_file_location = $_FILES['logo_img']['tmp_name'];
        $image_to_upload = $temp_file_location;
        ...
        if (!empty($image_url)) {
            connect($image_url, function ($image_url, $conn) {
                ...
                $res = $conn->query($q);
                if ($res->num_rows > 0) {
                    $q = "UPDATE `clients` SET `logo_img` = $image_url WHERE id = ".$client_id;
                    if ($conn->query($q)) {
                        $q = "SELECT " . implode(", ", $GLOBALS['get_fields']['clients']) . " FROM `clients` WHERE `id` = ".$client_id;
                        $res = $conn->query($q);
                        if ($res->num_rows > 0) {
                            response(200, "Updated Successfully", false, $res->fetch_assoc());
                        } else {
            ...
            response(503, "Failed to upload logo picture");
        }
    } else {
